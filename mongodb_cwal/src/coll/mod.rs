...
use ThreadedClient;
use common::{ReadPreference, WriteConcern, merge_options};
use cursor::Cursor;
use db::{Database, ThreadedDatabase};

use Error::{ArgumentError, BulkWriteError, DecoderError, OperationError, ResponseError};
use Result;
...
impl Collection {
    ...
    pub fn distinct(
        ...
        match result.get("values") {
            ...
            _ => Err(ResponseError(String::from(
                "No values received from server.",
            ))),
        }
    }
    ...
    pub fn find_one_with_command_type(
        ...
        let mut cursor = self.find_with_command_type(filter, Some(find_one_options), cmd_type)?;
    ...
    fn get_unordered_batches(requests: Vec<WriteModel>) -> Vec<Batch> {
        ...
        for req in requests {
            match req {
                ...
                WriteModel::DeleteOne { filter } => deletes.push(DeleteModel {
                    filter: filter,
                    multi: false,
                }),
                WriteModel::DeleteMany { filter } => deletes.push(DeleteModel {
                    filter: filter,
                    multi: true,
                }),
    ...
    fn execute_update_batch(
        ...
        match self.bulk_update(models, ordered, None, CommandType::UpdateMany) {
            Ok(bulk_update_result) => result.process_bulk_update_result(
                bulk_update_result,
                original_models,
                start_index,
                exception,
    ...
    pub fn insert_one(
        ...
        if ids.is_empty() {
            return Err(OperationError(String::from(
                "No ids returned for insert_one.",
            )));
        }
    ...
    pub fn insert_many(
        ...
        let write_concern = options.as_ref().and_then(|opts| opts.write_concern.clone());

        let (ids, exception) =
            self.insert(docs, options, write_concern, CommandType::InsertMany)?;

        let mut map = BTreeMap::from_iter(ids.into_iter().enumerate().map(|(k, v)| (k as i64, v)));
    ...
    fn delete(
        ...
        .map(DeleteResult::with_bulk_result)
    }
    ...
    fn update(
        ...
        .map(UpdateResult::with_bulk_result)
    }
        ...
        for key in replacement.keys() {
            if key.starts_with('$') {
                return Err(ArgumentError(String::from(
                    "Replacement cannot include $ operators.",
                )));
            }
        }
        Ok(())
