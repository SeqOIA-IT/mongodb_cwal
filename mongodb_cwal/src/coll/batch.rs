...
impl From<WriteModel> for Batch {
    fn from(model: WriteModel) -> Batch {
        match model {
            ...
            WriteModel::DeleteOne { filter } => Batch::Delete(vec![DeleteModel {
                filter: filter,
                multi: false,
            }]),
            WriteModel::DeleteMany { filter } => Batch::Delete(vec![DeleteModel {
                filter: filter,
                multi: true,
            }]),
...
impl Batch {
    ...
    pub fn merge_model(&mut self, model: WriteModel) -> Option<WriteModel> {
        match *self {
            Batch::Insert(ref mut docs) => match model {
                WriteModel::InsertOne { document } => docs.push(document),
                _ => return Some(model),
            },
            Batch::Delete(ref mut models) => match model {
                WriteModel::DeleteOne { filter } => models.push(DeleteModel {
                    filter: filter,
                    multi: false,
                }),
                WriteModel::DeleteMany { filter } => models.push(DeleteModel {
                    filter: filter,
                    multi: true,
                }),
                _ => return Some(model),
            },
            Batch::Update(ref mut models) => match model {
                WriteModel::ReplaceOne {
                    filter,
                    replacement: update,
                    upsert,
