...
use crate::{Client, wire_protocol::flags::OpQueryFlags};
...
impl Authenticator<'_> {
    ...
    fn next(&mut self, password: String, initial_data: InitialData) -> Result<AuthData> {
        ...
        pbkdf2_hmac::<HmacSha1>(hashed_password.as_bytes(), &salt, i, &mut salted_password);

        // Compute client key
        let mut client_key_hmac = HmacSha1::new_with_prefix(&salted_password);
        let client_key_bytes = b"Client Key";
        client_key_hmac.update(client_key_bytes);
        let client_key = client_key_hmac.finalize();

        // Hash into stored key
        let mut stored_key_sha1 = Sha1::new();
        stored_key_sha1.update(&client_key[..]);
        let stored_key = stored_key_sha1.finalize();
        ...
        let mut client_signature_hmac = HmacSha1::new_with_prefix(&stored_key);
        client_signature_hmac.update(auth_message.as_bytes());
        let client_signature = client_signature_hmac.finalize();
    ...
    fn finish(&mut self, conversation_id: Bson, auth_data: AuthData) -> Result<()> {
        ...
        // Compute server key
        let mut server_key_hmac = HmacSha1::new_with_prefix(&auth_data.salted_password);
        let server_key_bytes = b"Server Key";
        server_key_hmac.update(server_key_bytes);
        let server_key = server_key_hmac.finalize();

        // Compute server signature
        let mut server_signature_hmac = HmacSha1::new_with_prefix(&server_key);
        server_signature_hmac.update(auth_data.message.as_bytes());
        let server_signature = server_signature_hmac.finalize();
